...
- \( \beta_i \) are the coefficients estimated from data  
- \( XXXX_i \) are indicator variables (ANOVA encoding), where \( X_i = 1 \) if the corresponding factor is present and \( X_i = 0 \) otherwise. The first level of each categorical variable is assumed present by default and thus not listed.  
- \( \epsilon \sim N(0, \sigma^2) \) represents residual errors.  
...
We refine our model using stepwise selection based on Akaike’s Information Criterion (AIC). We see that the Size has a very large P value and thus can be removed from the model without compromising it's predictive power.
...
sigma_hat <- signif(summary(linear_model)$sigma, 2)  # Extract and round residual SD
...
# Add epsilon term with normal distribution
equation_text <- paste0(equation_text, "&+ \\epsilon, \\quad \\epsilon \\sim \\mathcal{N}(0, ", sigma_hat, "^2) \\end{aligned}")

# Print equation in R Markdown
